package com.oreilly.mrdp.appendixA;
...
public class BloomFilterDriver {
  ...
  public static void main(String[] args) throws Exception {
    ...
    System.out.println("Training Bloom filter of size " + vectorSize + " with "
        + nbHash + " hash functions, " + numMembers
    ...
    for (FileStatus status : fs.listStatus(inputFile)) {
      ...
      if (status.getPath().getName().endsWith(".gz")) {
        rdr = new BufferedReader(new InputStreamReader(new GZIPInputStream(
  ...
  public static int getOptimalBloomFilterSize(int numRecords, float falsePosRate) {
    int size = (int) (-numRecords * (float) Math.log(falsePosRate) / Math.pow(
        Math.log(2), 2));
    return size;
  }
